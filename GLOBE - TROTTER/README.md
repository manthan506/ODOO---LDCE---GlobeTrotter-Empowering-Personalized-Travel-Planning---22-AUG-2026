...
- **`app/`** — **Frontend Pages & View Layer** (Next.js 13 App Router, React Server/Client Components, Protected Route Wrappers).
- **`app/api/`** — **Backend REST API Routes** (Custom-built route handlers for Auth, Trips, Stops, Activities, Expenses, Budget calculations, and Public Sharing. Zero BaaS).
- **`components/`** — **Frontend UI Component Library** (Interactive widgets, Recharts donut visualization, slide-over sheets, travel style selectors, timeline cards).
- **`models/`** — **Backend Data Schemas & Mongoose Models** (`User`, `Trip`, `Stop`, `City`, `Activity`, `Expense`, `SharedTrip` with strict type definitions).
- **`lib/`** — **Backend & Utility Layer** (Cached Mongoose connection helper with in-memory local fallback, JWT cookie session helpers, auto-seeder).
- **`context/` & `hooks/`** — **Client State & API Data Layer** (`AuthContext`, `useTrips`, `useTrip`, `useCities`, `useExpenses` connecting directly to custom `/api/*` endpoints).
